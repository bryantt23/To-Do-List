...
addTask.addEventListener('click', function (event) {
  if (addTask.innerText === 'Save Edits') {
    deleteFlag = true;
  } else {
    deleteFlag = false;
  }
...
function handleEditTask(taskInfo, taskDiv) {
  ...
  editSubmit.addEventListener('click', function (event) {
    ...
    taskDetails.querySelector(
      '.cat-task'
    ).textContent = `Category: ${taskInfo.category}`;
    taskDetails.querySelector(
      '.title-task'
    ).textContent = `Title: ${taskInfo.title}`;
    taskDetails.querySelector(
      '.descript-task'
    ).textContent = `Description: ${taskInfo.description}`;
    taskDetails.querySelector(
      '.date-task'
    ).textContent = `Due Date: ${taskInfo.date}`;
    taskDetails.querySelector(
      '.priority-task'
    ).textContent = `Priority: ${taskInfo.priority}`;
    taskDetails.querySelector(
      '.notes-task'
    ).textContent = `Notes: ${taskInfo.notes}`;
  ...
  Work: [
    {
      ...
      date: '2023-11-29',
      priority: 'Low',
      notes: 'Use inside voice'
    },
    {
      category: 'Work',
      title: 'Email',
      description: 'Reply to Lenny',
      date: '2023-11-26',
      priority: 'Medium',
      notes: 'Go over chart again'
    }
  ],
  Home: [
    {
      category: 'Home',
      title: 'Tidy Up',
      description: 'Sister coming into town',
      date: '2023-11-30',
      priority: 'High',
      notes: 'Hide all of the cookies'
    }
  ],
  Mind: [
    {
      category: 'Mind',
      title: 'Study',
      description: 'Chapter 12-14',
      date: '2023-12-01',
      priority: 'Low',
      notes: 'Highlight key points'
    },
    {
      category: 'Mind',
  ...
  Miscellaneous: [
    {
      ...
      description: 'No more Amazon Prime',
      date: '2023-12-03',
      priority: 'High',
      notes: 'Only 5 days left'
    }
  ]
};
